...
func WriteLog(logType string, logDataType string, userId int, deletedData any) error {
	...
	// 현재 날짜 가져오기
	currentDate := time.Now().Format("060102") // YYMMDD 형식

	// 로그 파일 이름 만들기 (수정됨)
	fileName := fmt.Sprintf("%s/%d_%s.log", logFolder, userId, currentDate)
