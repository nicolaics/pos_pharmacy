...
func (h *Handler) handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		utils.WriteError(w, http.StatusBadRequest,
			fmt.Errorf("doctor with name %s already exists", payload.Name), nil)
		return
	}
	...
	utils.WriteSuccess(w, http.StatusCreated, fmt.Sprintf("doctor %s successfully created by %s", payload.Name, user.Name), nil)
}
...
func (h *Handler) handleGetAll(w http.ResponseWriter, r *http.Request) {
	...
	if val == "all" {
		...
		if err != nil {
			...
			if err != nil {
				utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("doctor %s not found", val), nil)
				return
			}
		} else {
			doctor, err := h.doctorStore.GetDoctorByID(id)
			if err != nil {
				utils.WriteError(w, http.StatusBadRequest, fmt.Errorf("doctor id %d not found", id), nil)
				return
			}
	...
	utils.WriteSuccess(w, http.StatusOK, doctors, nil)
}
...
func (h *Handler) handleGetOne(w http.ResponseWriter, r *http.Request) {
	...
	if doctor == nil || err != nil {
		utils.WriteError(w, http.StatusBadRequest,
			fmt.Errorf("doctor id %d doesn't exist", payload.ID), nil)
		return
	}

	utils.WriteSuccess(w, http.StatusOK, doctor, nil)
}
...
func (h *Handler) handleDelete(w http.ResponseWriter, r *http.Request) {
	...
	if doctor == nil || err != nil {
		utils.WriteError(w, http.StatusBadRequest,
			fmt.Errorf("doctor %s doesn't exist", payload.Name), nil)
		return
	}

	err = h.doctorStore.DeleteDoctor(doctor, user)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, err, nil)
		return
	}

	utils.WriteSuccess(w, http.StatusOK, fmt.Sprintf("doctor %s deleted by %s", payload.Name, user.Name), nil)
}
...
func (h *Handler) handleModify(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || doctor == nil {
		utils.WriteError(w, http.StatusBadRequest,
			fmt.Errorf("doctor with id %d doesn't exists", payload.ID), nil)
		return
	}
	...
	if err == nil {
		utils.WriteError(w, http.StatusBadRequest,
			fmt.Errorf("doctor with name %s already exist", payload.NewData.Name), nil)
		return
	}

	err = h.doctorStore.ModifyDoctor(doctor.ID, payload.NewData.Name, user)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, err, nil)
		return
	}

	utils.WriteSuccess(w, http.StatusCreated, fmt.Sprintf("doctor modified into %s by %s",
